...
using Polly.Retry;
using Polly.Timeout;
using WebApp.ProCodeGuide.Polly.Order.Models;

namespace WebApp.ProCodeGuide.Polly.Order.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        private string apiurl = @"http://localhost:5155/";
        ...
        public OrderDetails GetOrderByCustomerWithFallback(int customerCode)
        {
            ...
            //_fallbackPolicy.Wrap(_retryPolicy).Wrap(_timeoutPolicy).Wrap(_circuitBreakerPolicy).Execute(() => _httpClient.GetStringAsync(uri).Result);
            //_fallbackPolicy.Execute(() => _retryPolicy.Execute(() => _timeoutPolicy.Execute(() => _circuitBreakerPolicy.Execute(() => _httpClient.GetStringAsync(uri).Result))));
